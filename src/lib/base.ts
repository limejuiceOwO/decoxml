...
import { AttrDef, ChildDef, getMetadata, XMLMetadata } from './metadata';
import { isNil, mustBeTrue, notNil } from './utils';
...
export abstract class XMLElement {
  ...
  public toXmlObject(nsDef?: Record<string, string>): Element {
    ...
    const md = getMetadata(Object.getPrototypeOf(this));
    notNil(md, 'Tag must be defined on every node');
    ...
    for (const attrEntry of this.getAttrList()) {
      mustBeTrue(
        attrEntry.defined,
        `${md.name}.${attrEntry.prop}: @Attr must be defined`
      );
    ...
    for (const childDef of this.getChildList()) {
      ...
      if (isNil(rawChildObjs)) {
        mustBeTrue(
          childDef.minOccur === 0,
          `${md.name}.${childDef.prop}: missing child object`
        );
        continue;
      }
      ...
      if (Array.isArray(rawChildObjs)) {
        mustBeTrue(
          childDef.maxOccur !== 1,
          `${md.name}.${childDef.prop}: maxOccur must not equal to 1 if using array type`
        );
        mustBeTrue(
          childDef.maxOccur === 'unlimited' ||
            childDef.maxOccur >= rawChildObjs.length,
          `${md.name}.${childDef.prop}: too many child objects`
        );
        mustBeTrue(
          childDef.minOccur <= rawChildObjs.length,
          `${md.name}.${childDef.prop}: too few child objects`
        );
        ...
        mustBeTrue(
          childDef.maxOccur === 1,
          `${md.name}.${childDef.prop}: maxOccur must equal to 1 if not using array type`
        );
        childObjs = [rawChildObjs];
      } else {
        throw new Error(
      ...
      for (const childObj of childObjs) {
        mustBeTrue(
          childObj instanceof XMLElement,
          `${md.name}.${childDef.prop}[${index}]: not an XMLElement instance`
        );
    ...
    if (textDef) {
      ...
      if (!isNil(textProp)) {
        mustBeTrue(
          ...
          `${md.name}: text property must be string`
        );

        const textElem = doc.createTextNode(textProp as string);
        elem.appendChild(textElem);
      } else {
        mustBeTrue(!textDef.required, `${md.name}: missing text property`);
      }
    }
